...
- предварительно обученная смысловая модель (например, RuBERT)
  определяет смысловые категории сообщения;
- слой программных правил отвечает за однозначное извлечение сроков,
  финансов, задач, требований, событий и защитных признаков;
- слой контекста и состояния проекта связывает анализ с историей проекта.

Тот же контракт позволяет дальше развивать систему без переписывания ядра:
- подключить предварительно обученную многометочную модель;
- использовать большую языковую модель (LLM) для сложных требований
  и договорённостей;
- добавить поиск по истории и базе знаний (RAG);
- подключить отдельный модуль определения проекта;
- расширить анализ вложений.
...
class ExternalLanguageAnalyzer(Protocol):
    """
    Общий интерфейс внешнего языкового анализатора.

    Сейчас через этот контракт может подключаться RuBERT-адаптер.
    В дальнейшем тот же интерфейс можно использовать для других
    языковых моделей, не связывая AnalysisEngine с их реализацией.
    """
...
class AnalysisEngine:
    ...
    def _analyze_rules(
        ...
        """
        Базовый анализатор на программных правилах, без нейросетевой модели.

        Он извлекает однозначные рабочие признаки и остаётся частью
        гибридной архитектуры даже при подключённой языковой модели.
        """
    ...
    def _extract_requirements(
        ...
        Здесь используется консервативный слой программных правил.
        Более сложные формулировки должны разбираться смысловым слоем
        и отдельным модулем извлечения сущностей.
        """
    ...
    def _detect_conflict(
        ...
        """
        Оценка локального конфликта по программным правилам.

        Она относится только к текущему сообщению.
        Конфликт как состояние всего диалога должен определяться
        отдельным контекстным анализом истории проекта и переписки.
        """
    ...
    def _extract_deadline(
        ...
        должны обрабатываться отдельным модулем разбора даты и времени
    ...
    def _link_tasks_to_same_message_requirements(
        ...
        При нескольких требованиях связь не угадывается автоматически:
        её должен определять смысловой или контекстный слой.
        """
